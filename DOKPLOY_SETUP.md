...
2.  **Environment Variables**:
    ...
    *   *(Add any other secrets like KEEP_API_KEY if needed)*
...
chmod -R 777 /etc/dokploy/volumes/cleverprices/data
```
...
After setting up the app, it will be empty because the volume is empty.

**Push your local database to the production server:**
Run this from your **Local Machine (Mac)**:

```bash
bun run db:push-prod
```
*(This script builds `lite.db` locally and SCPs it to the host folder defined in Step 3.3)*.

Then **Redeploy** the app in Dokploy to pick up the data.
...
5.  [ ] Run `bun run db:push-prod` (Update IP in package.json script first!).
6.  [ ] Deploy.
